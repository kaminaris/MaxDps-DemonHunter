...
function Havoc:cooldown()
    ...
    if (MaxDps:CheckSpellUsable(classtable.SigilofSpite, 'SigilofSpite')) and (not debuff[classtable.EssenceBreakDeBuff].up and ( debuff[classtable.ReaversMarkDeBuff].remains >= 2 - (talents[classtable.QuickenedSigils] and talents[classtable.QuickenedSigils] or 0) or not (MaxDps.ActiveHeroTree == 'aldrachireaver') ) and not cooldown[classtable.BladeDance].ready and timeInCombat >15) and cooldown[classtable.SigilofSpite].ready then
...
function Havoc:meta()
    ...
    if (MaxDps:CheckSpellUsable(classtable.FelRush, 'FelRush')) and (buff[classtable.InertiaTriggerBuff].up and talents[classtable.Inertia] and cooldown[classtable.BladeDance].remains <gcd * 3 and ( not (MaxDps.ActiveHeroTree == 'felscarred') or not cooldown[classtable.EyeBeam].ready ) and not cooldown[classtable.Metamorphosis].ready) and cooldown[classtable.FelRush].ready then
        MaxDps:GlowCooldown(classtable.FelRush, cooldown[classtable.FelRush].ready)
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.SigilofDoom, 'SigilofDoom')) and (not cooldown[classtable.BladeDance].ready and not debuff[classtable.EssenceBreakDeBuff].up) and cooldown[classtable.SigilofDoom].ready then
        if not setSpell then setSpell = classtable.SigilofDoom end
    end
    if (MaxDps:CheckSpellUsable(classtable.ImmolationAura, 'ImmolationAura')) and (buff[classtable.DemonsurgeBuff].up and buff[classtable.DemonsurgeBuff].remains <gcd * 3 and buff[classtable.MetamorphosisBuff].up) and cooldown[classtable.ImmolationAura].ready then
        if not setSpell then setSpell = classtable.ImmolationAura end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.DeathSweep, 'DeathSweep')) and (( debuff[classtable.EssenceBreakDeBuff].up or cooldown[classtable.EssenceBreak].remains >= gcd * 2 or ( cooldown[classtable.Metamorphosis].ready and not cooldown[classtable.EyeBeam].ready and not talents[classtable.RestlessHunter] ) or not talents[classtable.EssenceBreak] or ( (MaxDps.spellHistory[1] == classtable.Metamorphosis) and talents[classtable.ChaoticTransformation] ) ) and talents[classtable.CycleofHatred]) and cooldown[classtable.DeathSweep].ready then
        if not setSpell then setSpell = classtable.DeathSweep end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.Annihilation, 'Annihilation')) and (not cooldown[classtable.BladeDance].ready or Fury >60 or SoulFragments >0 or buff[classtable.MetamorphosisBuff].remains <5 and cooldown[classtable.Felblade].ready) and cooldown[classtable.Annihilation].ready then
        if not setSpell then setSpell = classtable.Annihilation end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.ImmolationAura, 'ImmolationAura')) and (true and cooldown[classtable.ImmolationAura].duration <( math.min ( cooldown[classtable.EyeBeam].remains , buff[classtable.MetamorphosisBuff].remains ) ) and ( targets >= 1 + 1 or math.huge >cooldown[classtable.ImmolationAura].fullRecharge )) and cooldown[classtable.ImmolationAura].ready then
        if not setSpell then setSpell = classtable.ImmolationAura end
    end
...
function Havoc:opener()
    if (MaxDps:CheckSpellUsable(classtable.VengefulRetreat, 'VengefulRetreat')) and (talents[classtable.Initiative] and ( not buff[classtable.InitiativeBuff].up or talents[classtable.RestlessHunter] and buff[classtable.InitiativeBuff].remains <= 0.1 ) and timeInCombat >4 and ( ( talents[classtable.EssenceBreak] and not talents[classtable.RestlessHunter] and not cooldown[classtable.Metamorphosis].ready and not cooldown[classtable.EyeBeam].ready ) or ( talents[classtable.EssenceBreak] and talents[classtable.RestlessHunter] and ( not buff[classtable.MetamorphosisBuff].up or (MaxDps.spellHistory[1] == classtable.DeathSweep) ) and ( not cooldown[classtable.EyeBeam].ready or debuff[classtable.EssenceBreakDeBuff].up ) and buff[classtable.MetamorphosisBuff].up ) or not talents[classtable.EssenceBreak] )) and cooldown[classtable.VengefulRetreat].ready then
        MaxDps:GlowCooldown(classtable.VengefulRetreat, cooldown[classtable.VengefulRetreat].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.FelRush, 'FelRush')) and (talents[classtable.Inertia] and buff[classtable.InertiaTriggerBuff].up and talents[classtable.EssenceBreak] and not cooldown[classtable.Metamorphosis].ready and buff[classtable.DemonsurgeBuff].count == 4 and buff[classtable.MetamorphosisBuff].up) and cooldown[classtable.FelRush].ready then
        MaxDps:GlowCooldown(classtable.FelRush, cooldown[classtable.FelRush].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.Annihilation, 'Annihilation')) and (buff[classtable.MetamorphosisBuff].up and talents[classtable.RestlessHunter]) and cooldown[classtable.Annihilation].ready then
        if not setSpell then setSpell = classtable.Annihilation end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.FelRush, 'FelRush')) and (talents[classtable.Inertia] and buff[classtable.UnboundChaosBuff].up and not talents[classtable.AFireInside] and buff[classtable.MetamorphosisBuff].up and not cooldown[classtable.Metamorphosis].ready) and cooldown[classtable.FelRush].ready then
        MaxDps:GlowCooldown(classtable.FelRush, cooldown[classtable.FelRush].ready)
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.ImmolationAura, 'ImmolationAura')) and (talents[classtable.Inertia] and not buff[classtable.UnboundChaosBuff].up and buff[classtable.MetamorphosisBuff].up and not debuff[classtable.EssenceBreakDeBuff].up and not cooldown[classtable.BladeDance].ready and ( not buff[classtable.InnerDemonBuff].up or (MaxDps.ActiveHeroTree == 'felscarred') and not buff[classtable.MetamorphosisBuff].up ) and buff[classtable.DemonsurgeBuff].count <5) and cooldown[classtable.ImmolationAura].ready then
        if not setSpell then setSpell = classtable.ImmolationAura end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.Metamorphosis, 'Metamorphosis')) and (buff[classtable.MetamorphosisBuff].up and cooldown[classtable.BladeDance].remains >gcd * 2 and not buff[classtable.InnerDemonBuff].up and ( not (MaxDps.ActiveHeroTree == 'felscarred') and ( not talents[classtable.RestlessHunter] or (MaxDps.spellHistory[1] == classtable.DeathSweep) ) or buff[classtable.DemonsurgeBuff].count == 2 ) and ( not cooldown[classtable.EssenceBreak].ready or (MaxDps.ActiveHeroTree == 'felscarred') or talents[classtable.ShatteredDestiny] or not talents[classtable.EssenceBreak] )) and cooldown[classtable.Metamorphosis].ready then
        MaxDps:GlowCooldown(classtable.Metamorphosis, cooldown[classtable.Metamorphosis].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.SigilofSpite, 'SigilofSpite')) and ((MaxDps.ActiveHeroTree == 'felscarred') or debuff[classtable.ReaversMarkDeBuff].up and ( not talents[classtable.CycleofHatred] or not cooldown[classtable.EyeBeam].ready and not cooldown[classtable.Metamorphosis].ready )) and cooldown[classtable.SigilofSpite].ready then
        MaxDps:GlowCooldown(classtable.SigilofSpite, cooldown[classtable.SigilofSpite].ready)
    end
    if (MaxDps:CheckSpellUsable(classtable.SigilofDoom, 'SigilofDoom')) and (not buff[classtable.InnerDemonBuff].up and not debuff[classtable.EssenceBreakDeBuff].up and not cooldown[classtable.BladeDance].ready) and cooldown[classtable.SigilofDoom].ready then
        if not setSpell then setSpell = classtable.SigilofDoom end
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.AbyssalGaze, 'AbyssalGaze')) and (not debuff[classtable.EssenceBreakDeBuff].up and not cooldown[classtable.BladeDance].ready and not buff[classtable.InnerDemonBuff].up) and cooldown[classtable.AbyssalGaze].ready then
        if not setSpell then setSpell = classtable.AbyssalGaze end
    end
    if (MaxDps:CheckSpellUsable(classtable.EssenceBreak, 'EssenceBreak') and talents[classtable.EssenceBreak]) and (( cooldown[classtable.BladeDance].remains <gcd and not (MaxDps.ActiveHeroTree == 'felscarred') and not talents[classtable.ShatteredDestiny] and buff[classtable.MetamorphosisBuff].up or ( not cooldown[classtable.EyeBeam].ready or not buff[classtable.MetamorphosisBuff].up and ( buff[classtable.DemonsurgeBuff].count == 5 or buff[classtable.DemonsurgeBuff].count == 4 and not talents[classtable.InnerDemon] ) ) and not cooldown[classtable.Metamorphosis].ready ) and ( not (MaxDps.ActiveHeroTree == 'felscarred') or not buff[classtable.InnerDemonBuff].up )) and cooldown[classtable.EssenceBreak].ready then
        if not setSpell then setSpell = classtable.EssenceBreak end
    end
    if (MaxDps:CheckSpellUsable(classtable.EssenceBreak, 'EssenceBreak') and talents[classtable.EssenceBreak]) and (talents[classtable.RestlessHunter] and buff[classtable.MetamorphosisBuff].up and not buff[classtable.InnerDemonBuff].up and ( not (MaxDps.ActiveHeroTree == 'felscarred') or not buff[classtable.MetamorphosisBuff].up )) and cooldown[classtable.EssenceBreak].ready then
        if not setSpell then setSpell = classtable.EssenceBreak end
    end
...
function Havoc:callaction()
    ...
    fel_barrage = talents[classtable.FelBarrage] and ( cooldown[classtable.FelBarrage].remains <gcd * 7 and ( targets >= 1 + 1 or math.huge <gcd * 7 or math.huge >90 ) and ( not cooldown[classtable.Metamorphosis].ready or targets >2 ) or buff[classtable.FelBarrageBuff].up ) and not ( targets == 1 and (targets <2) )
    ...
    if (MaxDps:CheckSpellUsable(classtable.SigilofSpite, 'SigilofSpite')) and (not debuff[classtable.EssenceBreakDeBuff].up and debuff[classtable.ReaversMarkDeBuff].remains >= 2 - (talents[classtable.QuickenedSigils] and talents[classtable.QuickenedSigils] or 0)) and cooldown[classtable.SigilofSpite].ready then
        MaxDps:GlowCooldown(classtable.SigilofSpite, cooldown[classtable.SigilofSpite].ready)
    end
    ...
    if (MaxDps:CheckSpellUsable(classtable.ImmolationAura, 'ImmolationAura')) and ((MaxDps.ActiveHeroTree == 'felscarred') and cooldown[classtable.Metamorphosis].remains <10 and cooldown[classtable.EyeBeam].remains <10 and ( not buff[classtable.UnboundChaosBuff].up or cooldown[classtable.FelRush].charges == 0 or ( math.min ( cooldown[classtable.EyeBeam].remains , cooldown[classtable.Metamorphosis].remains ) ) <5 ) and talents[classtable.AFireInside]) and cooldown[classtable.ImmolationAura].ready then
        if not setSpell then setSpell = classtable.ImmolationAura end
    end
...
function DemonHunter:Havoc()
    ...
    classtable.ReaversGlaiveBuff = 444686
