...
function Havoc:ar()
    ...
    if (MaxDps:CheckSpellUsable(classtable.VengefulRetreat, 'VengefulRetreat')) and (talents[classtable.Initiative] and not talents[classtable.TacticalRetreat] and (cooldown[classtable.EyeBeam].remains >15 and MaxDps:CooldownConsolidated(61304).remains <0.3 or MaxDps:CooldownConsolidated(61304).remains <0.2 and cooldown[classtable.EyeBeam].remains <= MaxDps:CooldownConsolidated(61304).remains and cooldown[classtable.Metamorphosis].remains >10) and (not trinket1_steroids and not trinket2_steroids or trinket1_steroids and ((MaxDps:CheckTrinketStat('13', 'any') and MaxDps:CheckTrinketCooldown('13') or 0) <gcd*3 or (MaxDps:CheckTrinketStat('13', 'any') and MaxDps:CheckTrinketCooldown('13') or 0) >30) or trinket2_steroids and ((MaxDps:CheckTrinketStat('14', 'any') and MaxDps:CheckTrinketCooldown('14') or 0) <gcd*3 or (MaxDps:CheckTrinketStat('14', 'any') and MaxDps:CheckTrinketCooldown('14') or 0) >30)) and timeInCombat >20 and (not talents[classtable.Inertia] and not buff[classtable.UnboundChaosBuff].up or not buff[classtable.InertiaTriggerBuff].up and not buff[classtable.MetamorphosisBuff].up)) and cooldown[classtable.VengefulRetreat].ready then
        MaxDps:GlowCooldown(classtable.VengefulRetreat, cooldown[classtable.VengefulRetreat].ready)
    end
...
function Havoc:callaction()
    ...
    trinket_pacemaker_proc = MaxDps:CheckTrinketNames('ImprovisedSeaforiumPacemaker') and ((MaxDps:CheckTrinketStat('13', 'crit') and MaxDps:CheckTrinketCooldown('13')>0) and true or false) or MaxDps:CheckTrinketNames('ImprovisedSeaforiumPacemaker') and ((MaxDps:CheckTrinketStat('14', 'crit') and MaxDps:CheckTrinketCooldown('14')>0) and true or false) or not MaxDps:CheckEquipped('ImprovisedSeaforiumPacemaker')
